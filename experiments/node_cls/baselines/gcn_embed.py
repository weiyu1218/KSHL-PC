"""
GCN (Graph Convolutional Network) 宓屽叆鐢熸垚 - 绾疨yTorch鐗堟湰
"""
import torch
...
def train_gcn(x, adj, in_dim, hidden_dim, out_dim, num_layers=2, dropout=0.5,
              lr=0.01, epochs=200, device='cpu', verbose=False):
    """
    璁粌GCN妯″瀷鐢熸垚鑺傜偣宓屽叆
    """
    model = GCNEncoder(in_dim, hidden_dim, out_dim, num_layers, dropout).to(device)
    x = x.to(device)
    adj = adj.to(device)

    optimizer = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=5e-4)

    # 鍒涘缓璁粌鐢ㄧ殑杈癸紙鐢ㄤ簬閾捐矾棰勬祴锛?
    adj_dense = adj.to_dense()
    pos_edges = (adj_dense > 0).nonzero(as_tuple=False)

    def link_prediction_loss(z, pos_edges, num_nodes):
        # 姝ｆ牱鏈?
        pos_idx = torch.randperm(pos_edges.size(0))[:1000]  # 閲囨牱1000鏉℃杈?
...
def main():
    parser = argparse.ArgumentParser(description='Generate GCN embeddings')
    ...
    print(f"Training GCN model on {args.device}...")

    embeddings = train_gcn(
        x, adj_norm,
        in_dim=x.size(1),
