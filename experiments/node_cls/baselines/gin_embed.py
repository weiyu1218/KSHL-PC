"""
GIN (Graph Isomorphism Network) 宓屽叆鐢熸垚 - 绾疨yTorch鐗堟湰
"""
import torch
...
class GINEncoder(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim, num_layers=2, dropout=0.5):
        ...
        self.convs.append(GINConv(hidden_dim, out_dim))
...
def train_gin(x, adj, in_dim, hidden_dim, out_dim, num_layers=2, dropout=0.5,
              lr=0.01, epochs=200, device='cpu', verbose=False):
    """
    璁粌GIN妯″瀷鐢熸垚鑺傜偣宓屽叆
    """
    model = GINEncoder(in_dim, hidden_dim, out_dim, num_layers, dropout).to(device)
    x = x.to(device)
    adj = adj.to(device)
...
def main():
    parser = argparse.ArgumentParser(description='Generate GIN embeddings')
    ...
    print(f"Training GIN model on {args.device}...")

    embeddings = train_gin(
        x, adj_norm,
        in_dim=x.size(1),
