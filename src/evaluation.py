...
def calculate_fmax(preds, labels):
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        for t in range(1, 100):
            ...
            sn = tp / (1.0 * np.sum(labels)) if np.sum(labels) > 0 else 0

            sp_denom = np.sum(labels ^ 1)
            sp = np.sum((predictions ^ 1) * (labels ^ 1)) / sp_denom if sp_denom > 0 else 0
    ...
    return round(f_max, 3), round(t_max, 3), round(p_max, 3), round(r_max, 3), round(r_max, 3), round(sp_max, 3), round(acc_max, 3)
